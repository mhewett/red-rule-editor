$(document).ready(function() {
    ...
    function parse() {
        oldInput = $("#grammar").val();

        $("#input").removeAttr("disabled");
        $("#parse-message").attr("class", "message progress").text("Parsing the input...");
        $("#input").addClass("disabled").text("Output not available.");

        try {
            var timeBefore = (new Date).getTime();
            var output = parser.parse($("#grammar").val());
            var timeAfter = (new Date).getTime();
            ...
            // output is an array with one string in it.
            output = JSON.parse(output[0]);
            $("#input").removeClass("disabled").text(jsDump.parse(output));
    ...
    $("#loader").hide();
    $("#content").show();
    $('#build-message').attr("class", "message info").text("Ready");

    //$("#grammar, #parser-var").removeAttr("disabled");
    $("#grammar").removeAttr("disabled");
